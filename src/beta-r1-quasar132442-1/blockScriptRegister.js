...

{
    ...
    let clearObject = () => {
        ...
        while (Ent3DGLRenderingHelper.scene.children.length) {
            Ent3DGLRenderingHelper.scene.remove(Ent3DGLRenderingHelper.scene.children[0])
        }
    }
    let throwError = msg => {
    ...
    Ent3D.Ent3DScripts = {
        ...
        Ent3D_setCameraRotation(sprite, script) {
            ...
            if(isNaN(VALUE)) return;
            Ent3DGLRenderingHelper.camera.rotation[DIRECTION] = Math.radians(angleNormalization(VALUE));
        },
        ...
        Ent3D_createCubeSixColor(sprite, script) {
            const OBJNAME = script.getValue('OBJNAME', script);
            const COLORS = new Array(6).fill(0).map((_, index) => script.getValue(`COLOR${index}`, script))
            if (objectExist(OBJNAME)) return throwError(`'${OBJNAME}'의 이름을 가진 오브젝트가 이미 존재합니다.`);
            const geometry = new THREE.BoxGeometry();
            const materials = new Array(6).fill(0).map((_, index) => new THREE.MeshBasicMaterial({ color: COLORS[index] }));
        ...
        Ent3D_setObjectRotation(sprite, script) {
            ...
            if(!objectExist(OBJNAME)) return throwError(`'${OBJNAME}' 이름의 오브젝트를 찾을수 없습니다.`);
            if(isNaN(VALUE)) return;
            findObject(OBJNAME).rotation[DIRECTION] = Math.radians(angleNormalization(VALUE));
        },
        Ent3D_adjustObjectRotation(sprite, script) {
            ...
            if(!objectExist(OBJNAME)) return throwError(`'${OBJNAME}' 이름의 오브젝트를 찾을수 없습니다.`);
            if(isNaN(VALUE)) return;
            findObject(OBJNAME).rotation[DIRECTION] = findObject(OBJNAME).rotation[DIRECTION] + Math.radians(angleNormalization(VALUE));
        },
